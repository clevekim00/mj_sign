...
작성일: 2026-07-25  
검토 범위: `slr_input_kit`, `sign_bridge`, `sign_gemma_mock`, 공통 protobuf 및 핵심 아키텍처/API 문서
...
“Gemma 4를 내장해 수어를 직접 인식한다”는 표현은 아직 사용하면 안 된다. 현재 Gemma 계열 LLM은 주로 **인식 모델이 출력한 키워드를 자연스러운 문장으로 보정**하는 역할이며, `sign_gemma_mock`의 real 모드도 실제 landmark-to-text 시각 모델 대신 프로필에 정의된 키워드 힌트를 LLM에 전달한다.
...
- 공통 wire schema: `schema/landmark.proto`
...
3. **Gemma는 시각 수어 인식 모델이 아니다.**
   현재 `GemmaService`는 텍스트 키워드를 문장으로 다듬는다. `sign_gemma_mock/sign_gemma_model.py`도 landmark tensor를 모델에 넣지 않고 텍스트 prompt를 생성한다.
...
6. **클라이언트 테스트가 없다.**
   `slr_input_kit/test`가 존재하지 않는다. 카메라 lifecycle, 좌우 반전, reconnect, 이벤트 parsing, 프레임 제한 및 로컬 추론 fallback을 검증해야 한다.
...
- 백엔드: `sign_bridge/./gradlew test` 성공
- 클라이언트: `slr_input_kit/test` 디렉터리 없음
- 클라이언트 정적 분석: 로컬 Xcode 라이선스 미동의로 `dart analyze` 실행 차단
