...
공통 런타임 구조와 SignGemma-compatible 예제 흐름은 [PROJECT_ARCHITECTURE_KO.md](../backend/PROJECT_ARCHITECTURE_KO.md)에 정리되어 있습니다. Spring Boot + cross-platform app 실행 가이드는 [SIGN_GEMMA_APP_DEMO_KO.md](../backend/SIGN_GEMMA_APP_DEMO_KO.md)를 참고하세요.
...
- `sample/backend/model_server/profile_registry.py`의 English/ASL profile registry.
- `sample/backend/model_server/sign_gemma_model.py`의 profile별 Keras/Gemma engine cache.
- `sample/backend/model_server/main.py`의 profile-aware `/health`, `/ready` endpoint.
