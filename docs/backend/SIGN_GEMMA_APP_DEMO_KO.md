...
Fixture 데이터는 `sign/common/eval/fixtures/signbridge_eval_fixtures.json`에 있습니다.
...
| Mock 서버가 protobuf runtime version 오류로 실패 | `./scripts/setup_mock_venv.sh` 실행. smoke script는 `sample/backend/model_server/.venv`가 있으면 자동으로 사용 |
