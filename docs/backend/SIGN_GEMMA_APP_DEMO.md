...
Fixture data lives in `sign/common/eval/fixtures/signbridge_eval_fixtures.json`.
...
| Mock server fails with protobuf runtime version errors | Run `./scripts/setup_mock_venv.sh`; the smoke script automatically uses `sample/backend/model_server/.venv` when present |
