...
이 문서는 [PROJECT_PROMOTION_KO.md](../embedded/PROJECT_PROMOTION_KO.md)를 기반으로 SNS에 바로 올릴 수 있는 홍보 문안을 정리합니다. 현재 데모는 공식 SignGemma weight를 포함하지 않으며, SignGemma-compatible profile과 mock serving backend를 통해 앱-브릿지-모델 서버 계약을 검증하는 단계입니다.
